...
public class RequestProcessor {
	...
	public Mono<ResponseEntity<Publisher<?>>> post(FunctionWrapper wrapper, String body,
			boolean stream) {
	...
	private Mono<ResponseEntity<Publisher<?>>> post(FunctionWrapper wrapper, List<?> body,
	...
	public Mono<ResponseEntity<Publisher<?>>> stream(FunctionWrapper request) {
		Publisher<?> result;
	...
	private Mono<ResponseEntity<Publisher<?>>> stream(FunctionWrapper request, Publisher<?> result) {
	...
	private Mono<ResponseEntity<Publisher<?>>> response(Object handler, Publisher<?> result,
		...
		if (output instanceof Mono) {
			return Mono.from(output).then(Mono.fromSupplier(() -> builder.body(output)));
		}
		return Flux.from(output).then(Mono.fromSupplier(() -> builder.body(output)));
	}
	...
	public Mono<ResponseEntity<Publisher<?>>> get(FunctionWrapper wrapper) {
